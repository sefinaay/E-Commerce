...
@section('head')
<style>
.admin-wrap{max-width:1200px;margin:1rem auto;padding:0 1rem;}
.admin-tabs{display:flex;gap:.5rem;margin-bottom:1.5rem;border-bottom:2px solid var(--border);padding-bottom:0;}
.tab-btn{padding:.6rem 1.25rem;border:none;background:transparent;cursor:pointer;font-family:inherit;font-size:.9rem;color:#666;border-bottom:3px solid transparent;margin-bottom:-2px;transition:.2s;}
.tab-btn.active{color:var(--rose);border-bottom-color:var(--rose);font-weight:500;}
.tab-content{display:none;}
.tab-content.active{display:block;}
.stat-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:2rem;}
.stat-card{background:white;border:1px solid var(--border);border-radius:12px;padding:1.5rem;text-align:center;}
.stat-num{font-family:'Cormorant Garamond',serif;font-size:2.5rem;color:var(--rose);font-weight:600;}
.stat-label{font-size:.8rem;color:#999;margin-top:.25rem;}
...
.data-table th{background:var(--soft);padding:.75rem 1rem;text-align:left;font-size:.8rem;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#666;}
.data-table td{padding:.75rem 1rem;font-size:.85rem;border-bottom:1px solid var(--border);}
.status-badge{padding:.2rem .6rem;border-radius:50px;font-size:.7rem;font-weight:600;}
...
.form-input{width:100%;padding:.6rem 1rem;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:.85rem;margin-top:.25rem;}
</style>
@endsection
@section('content')
<div class="admin-wrap">
  ...
  <div class="tab-content active" id="tab-dashboard">
    ...
    <table class="data-table" id="recent-orders-table">
      <thead><tr><th>No. Pesanan</th><th>Customer</th><th>Total</th><th>Status</th><th>Tanggal</th></tr></thead>
  ...
  <div class="tab-content" id="tab-orders">
    ...
    <table class="data-table">
      <thead><tr><th>No. Pesanan</th><th>Customer</th><th>Total</th><th>Status</th><th>Pembayaran</th><th>Aksi</th></tr></thead>
  ...
  <div class="tab-content" id="tab-products">
    ...
    <div id="product-form-container" style="display:none;background:white;border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-bottom:1.5rem">
      <h4 style="margin-bottom:1rem">Tambah/Edit Produk</h4>
      <div class="form-row">
        <div><label style="font-size:.8rem;font-weight:600;color:#666">Nama Produk</label><input class="form-input" id="pf-name" placeholder="Nama produk"></div>
        <div><label style="font-size:.8rem;font-weight:600;color:#666">Brand</label><input class="form-input" id="pf-brand" placeholder="Brand"></div>
      </div>
      <div class="form-row" style="margin-top:.75rem">
        <div><label style="font-size:.8rem;font-weight:600;color:#666">Harga (Rp)</label><input class="form-input" id="pf-price" type="number" placeholder="0"></div>
        <div><label style="font-size:.8rem;font-weight:600;color:#666">Stok</label><input class="form-input" id="pf-stock" type="number" placeholder="0"></div>
      </div>
      ...
      <div style="margin-top:.75rem">
        ...
        <textarea class="form-input" id="pf-desc" rows="3" style="resize:none" placeholder="Deskripsi produk..."></textarea>
      </div>
      <div style="display:flex;gap:.5rem;margin-top:1rem">
        <button class="btn btn-primary" onclick="saveProduct()">Simpan</button>
        <button class="btn btn-outline" onclick="document.getElementById('product-form-container').style.display='none'">Batal</button>
      </div>
    </div>
    <table class="data-table">
      <thead><tr><th>Gambar</th><th>Nama</th><th>Brand</th><th>Harga</th><th>Stok</th><th>Status</th><th>Aksi</th></tr></thead>
  ...
  <div class="tab-content" id="tab-users">
    ...
    <table class="data-table">
      <thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>HP</th><th>Bergabung</th></tr></thead>
  ...
  <div class="tab-content" id="tab-import">
    <div style="background:white;border:1px solid var(--border);border-radius:12px;padding:2rem;max-width:500px">
      ...
      <p style="font-size:.85rem;color:#666;margin-bottom:1.5rem">Import produk langsung dari makeup-api.herokuapp.com ke database lokal.</p>
      <div style="margin-bottom:1rem">
        <label style="font-size:.8rem;font-weight:600;color:#666;display:block;margin-bottom:.4rem">Brand</label>
        <input class="form-input" id="imp-brand" placeholder="Maybelline, NYX, Revlon, Covergirl...">
      </div>
      <div style="margin-bottom:1rem">
        <label style="font-size:.8rem;font-weight:600;color:#666;display:block;margin-bottom:.4rem">Kategori Tujuan</label>
        <select class="form-input" id="imp-cat"></select>
      </div>
      <button class="btn btn-primary" onclick="importFromApi()" style="width:100%">Import Sekarang (maks 10 produk)</button>
