...
@section('head')
<style>
.discover-wrap{max-width:1100px;margin:0 auto;padding:2rem 1rem;}
.discover-hero{text-align:center;padding:3rem 1rem;background:linear-gradient(135deg,var(--soft),#fce4ec);border-radius:16px;margin-bottom:2rem;}
.discover-hero h1{font-family:'Cormorant Garamond',serif;font-size:3rem;font-weight:300;margin-bottom:.5rem;}
.discover-hero p{color:#666;}
.api-badge{display:inline-block;background:var(--gold);color:white;padding:.25rem .75rem;border-radius:50px;font-size:.75rem;font-weight:600;margin-bottom:1rem;}
.filter-row{display:flex;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap;}
.filter-row select,.filter-row input{padding:.6rem 1rem;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:.85rem;background:white;}
.ext-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:1.5rem;}
.ext-card{background:white;border-radius:12px;border:1px solid var(--border);overflow:hidden;transition:.2s;}
.ext-card:hover{transform:translateY(-4px);box-shadow:0 8px 24px rgba(0,0,0,.1);}
.ext-card img{width:100%;height:180px;object-fit:contain;padding:1rem;background:#f9f9f9;}
.ext-info{padding:1rem;}
.ext-brand{font-size:.7rem;color:var(--gold);font-weight:600;text-transform:uppercase;}
.ext-name{font-size:.9rem;font-weight:500;margin:.2rem 0;}
.ext-price{color:var(--rose);font-weight:600;}
.ext-rating{font-size:.8rem;color:#f59e0b;}
</style>
@endsection
@section('content')
<div class="discover-wrap">
  ...
  <div class="ext-grid" id="ext-grid"><p style="color:#999">Memuat dari Makeup API...</p></div>
  ...
  <div style="background:var(--soft);border-radius:12px;padding:1.5rem;margin-top:2rem;font-size:.85rem;color:#666;">
    ...
    Data di atas berasal langsung dari <strong>Makeup API (makeup-api.herokuapp.com)</strong> — sebuah REST API publik gratis untuk data produk makeup global. Data diambil real-time dan di-cache selama 5 menit.
  </div>
</div>
@endsection
@section('scripts')
